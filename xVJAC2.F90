PROGRAM xVJAC2
  ...
  INTEGER :: M, N
  CHARACTER(LEN=256) :: F
  INTEGER :: LDG, LDV, MAXCYC(3), MAXTHR(3), BLKTHR(2), JSTRAT(3), LWORK, LIWORK
...
!IBM* ALIGN(MALIGN_B, G,V, SIGMA, WORK,IWORK, WORK1,IWORK1)
  ...
  CALL READCL(M, N, JSTRAT(3), MAXCYC(3), MAXTHR(3), JSTRAT(2), MAXCYC(2), MAXTHR(2), BLKTHR(2),&
       JSTRAT(1), MAXTHR(1), BLKTHR(1), F, INFO(2))
  IF (INFO(2) .NE. 0) STOP 'xVJAC2.exe M N J0 C0 T0 J1 C1 T1 B1 J2 T2 B2 F'
  IF (M .LE. 0) STOP 'M <= 0'
  IF (N .LE. 0) STOP 'N <= 0'
  IF (N .GT. M) STOP 'N > M'
  MAXCYC(1) = MXCYC2
  ...
  LWORK = 0
  LIWORK = 0
  CALL VJAC2(FAST, M, N, G, LDG, V, LDV, MAXCYC, MAXTHR, BLKTHR, JSTRAT, TOL, SIGMA,&
       WORK1, LWORK, IWORK1, LIWORK, NROT, INFO)
  IF (INFO(1) .LT. 0) STOP 'VJAC2(workspace query)'
  IF (INFO(1) .GT. 0) THEN
     LWORK = INFO(1)
...
CONTAINS
  ...
  SUBROUTINE READCL(M, N, J0, C0, T0, J1, C1, T1, B1, J2, T2, B2, F, INFO)

    INTEGER, INTENT(OUT) :: M, N, J0, C0, T0, J1, C1, T1, B1, J2, T2, B2, INFO
    ...
    IF (INFO .EQ. 13) THEN
       ...
       CALL GET_COMMAND_ARGUMENT(12, F)
       READ (F,*) B2
