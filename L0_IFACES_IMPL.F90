SUBROUTINE DJACV0_PREPARE_F(STRAT, LDA, M, N, TOL, IWORK, LIWORK, INFO)
  IMPLICIT NONE

  INTEGER, INTENT(IN) :: STRAT, LDA, M, N, LIWORK
...
!IBM* ALIGN(MALIGN_B, JS, PIV)
  ...
  !DIR$ ASSUME_ALIGNED IWORK:MALIGN_B
  ...
  IF (LIWORK .EQ. -1) THEN
     INFO = 2*MINWRK
     RETURN
  ELSE IF (LIWORK .LT. (2*MINWRK)) THEN
     INFO = -7
     RETURN
  END IF
...
SUBROUTINE DJACV0_EXECUTE_F(LDA, M, N, G, V, TOL, SWP, IWORK, INFO)
  ...
  !DIR$ ASSUME_ALIGNED G,V,IWORK:MALIGN_B
  !DIR$ ASSUME (MOD(LDA,D_VEC_LEN) .EQ. 0)
