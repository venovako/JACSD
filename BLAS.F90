INTEGER FUNCTION BLAS_PREPARE()
  ...
  IF (.NOT. OMP_GET_DYNAMIC()) CALL OMP_SET_DYNAMIC(.TRUE.)
#ifdef USE_MKL
  IF (MKL_GET_DYNAMIC() .EQ. 0_c_int) CALL MKL_SET_DYNAMIC(1)
#ifndef NDEBUG
...
#ifdef USE_MKL
  BLAS_SET_NUM_THREADS = MKL_SET_NUM_THREADS_LOCAL(INT(NT,c_int))
#else
