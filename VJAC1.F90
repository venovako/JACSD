SUBROUTINE INVJAC1(NC1,IFC1, NC2,IFC2, M,N, G,LDG, V,LDV, NBSIZ2,LDAC,&
...
END SUBROUTINE INVJAC1

SUBROUTINE MYVJAC1(FAST, M, N, G, LDG, V, LDV, MAXCYC, MAXTHR, BLKTHR, NBSIZE, NBSIZ2, LDAC, NPAIRS, NBL, JSTRAT, TOL,&
     GVB, WORK, LWORK, IWORK, LIWORK, NC, IFC, JPAIRS, NROT, INFO)

  IMPLICIT NONE

  LOGICAL, INTENT(IN) :: FAST
  INTEGER, INTENT(IN) :: M,N, LDG,LDV, MAXCYC(2),MAXTHR(2),BLKTHR, NBSIZE,NBSIZ2,LDAC, NPAIRS,NBL, JSTRAT(2), LWORK,LIWORK
  INTEGER, INTENT(OUT) :: IWORK(LIWORK,MAXTHR(1)), NC(NBL),IFC(NBL), JPAIRS(2,NPAIRS), NROT(2),INFO(2)
  DOUBLE PRECISION, INTENT(IN) :: TOL
  ...
  INTEGER :: JS(JSMLEN), LROT(2), LROTP(2),INFOP(2), STEPS, CYC, STP, BLASNT, PAIR, B,P,Q, I,J,K, NC1,IFC1, NC2,IFC2, U, CLK(3)

  EXTERNAL :: DLACPY, DLASET

  !DIR$ ASSUME_ALIGNED G:MALIGN_B,V:MALIGN_B, GVB:MALIGN_B, WORK:MALIGN_B,IWORK:MALIGN_B
  !DIR$ ASSUME_ALIGNED NC:MALIGN_B,IFC:MALIGN_B, JPAIRS:MALIGN_B
  !DIR$ ASSUME (MOD(LDG,D_CL1_LEN) .EQ. 0)
  !DIR$ ASSUME (MOD(LDV,D_CL1_LEN) .EQ. 0)
  !DIR$ ASSUME (MOD(LDAC,D_CL1_LEN) .EQ. 0)
  !DIR$ ASSUME (MOD(LWORK,D_CL1_LEN) .EQ. 0)
  ...
  DO CYC = 1, MAXCYC(1)
     ...
     DO STP = 1, STEPS
        ...
        DO B = 1, BLKTHR
           ...
           Q = P + (MAXTHR(1) - 1)
           LROTP = 0
           !$OMP PARALLEL NUM_THREADS(MAXTHR(1)), PRIVATE(PAIR,I,J,K,NC1,IFC1,NC2,IFC2,INFOP), REDUCTION(+:LROTP)
           ...
           DO PAIR = P, Q
              ...
              CALL INVJAC1(NC1,IFC1, NC2,IFC2, M,N, G,LDG, V,LDV, NBSIZ2,LDAC,&
                   ...
                   WORK(1,K),LWORK, IWORK(1,K),LIWORK, LROTP,INFOP)

              IF (INFOP(1) .NE. 0) THEN
                 !$OMP CRITICAL
                 INFO = INFOP
...
END SUBROUTINE MYVJAC1

SUBROUTINE VJAC1(FAST, M, N, G, LDG, V, LDV, MAXCYC, MAXTHR, BLKTHR, JSTRAT, TOL, SIGMA,&
     WORK, LWORK, IWORK, LIWORK, NROT, INFO)

  IMPLICIT NONE

  LOGICAL, INTENT(IN) :: FAST
  INTEGER, INTENT(IN) :: M, N, LDG, LDV, MAXCYC(2), MAXTHR(2), BLKTHR, JSTRAT(2), LWORK, LIWORK
  ...
  INTEGER :: I_GVB,I_WRK, I_NC,I_IFC,I_JP,I_IWRK
  INTEGER :: L_GVB,L_WRK, L_NC,L_IFC,L_JP,L_IWRK, L_WORK,L_IWORK
  ...
  IF (N .EQ. 1) THEN
     CALL VJAC0(FAST, M, N, G, LDG, V, LDV, MAXCYC(2), MAXTHR(2), JSTRAT(2), TOL, SIGMA,&
  ...
  ELSE
     CALL VJAC0(.TRUE., NBSIZ2, NBSIZ2, G, LDAC, V, LDAC, MAXCYC(2), MAXTHR(2), JSTRAT(2), TOL, SIGMA,&
          WORK, MYLWRK, IWORK, MYLIWRK, NROT, INFO)
     IF (INFO(1) .LT. 0) RETURN
     ...
     MYLIWRK = L_NC + L_IFC + L_JP + L_IWRK
  END IF
  ...
  CALL MYVJAC1(FAST, M, N, G, LDG, V, LDV, MAXCYC, MAXTHR, BLKTHR, NBSIZE, NBSIZ2, LDAC, NPAIRS, MYNBL, JSTRAT, TOL,&
       WORK(I_GVB), WORK(I_WRK), L_WORK, IWORK(I_IWRK), L_IWORK,&
       IWORK(I_NC), IWORK(I_IFC), IWORK(I_JP), NROT, INFO)
