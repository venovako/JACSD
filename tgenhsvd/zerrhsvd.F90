...
CONTAINS
  ...
  SUBROUTINE READCL(FN, M, N, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 3) ERROR STOP 'zerrhsvd.exe FN M N'
  ...
  SUBROUTINE BREAD_Y(FD, Y, M, N, SZ, INFO)
    USE, INTRINSIC :: ISO_C_BINDING
    IMPLICIT NONE
    ...
    !$OMP DO
    DO J = 1, N
       I = INT(PVN_BREAD(INT(FD,c_int), Y(1,J), INT(SZ,c_size_t), INT(((J-1) * SZ),c_size_t)))
  ...
  SUBROUTINE BREAD_ZZ(FD, ZZ, N, SZ, INFO)
    ...
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: FD, N
    COMPLEX(KIND=c_double), INTENT(OUT) :: ZZ(N,N)
    INTEGER, INTENT(OUT) :: SZ, INFO

    INTEGER :: I, J
    INTEGER(KIND=c_size_t), EXTERNAL :: PVN_BREAD

    SZ = INT(N * C_SIZEOF(Z_ZERO))
    INFO = 0
