...
#include "qx_wp.fi"
  ...
  REAL(WP), PARAMETER :: Q_ZERO = 0.0_WP, Q_ONE = 1.0_WP, Q_MONE = -1.0_WP
...
#include "qx_wp.fi"
  ...
  REAL(WP), PARAMETER :: Q_ZERO = 0.0_WP

  CHARACTER, INTENT(IN) :: SIDE
  INTEGER, INTENT(IN) :: INCX, INCY, N
  REAL(WP), INTENT(IN) :: C1, C2
  COMPLEX(WP), INTENT(IN) :: S1, S2
  COMPLEX(WP), INTENT(INOUT) :: ZX(*), ZY(*)
  INTEGER, INTENT(OUT) :: INFO
...
#include "qx_wp.fi"
  ...
  COMPLEX(WP), INTENT(INOUT) :: A(LDA,N)
...
SUBROUTINE XHEJF2(N, A, LDA, JVEC, IPIV, INFO)
  ...
  !
  !     A       (input/output) COMPLEX(WP) array, dimension (LDA,N)
...
#include "qx_wp.fi"
  ...
  REAL(WP) :: DIAMAX, OFFMAX, R1, R2, C, TEMP, SAR1, SAR2, UD11, UD22
  COMPLEX(WP) :: S, UD21, UD12
...
#include "qx_wp.fi"
  ...
  COMPLEX(WP), INTENT(INOUT) :: G(LDG,NCOLR)
