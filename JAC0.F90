...
#else
SUBROUTINE MYDJAC0(NPAIRS, VPAIRS, VTORS, VTORSV, M, N, G, LDG, V, LDV, MAXCYC, MAXTHR, JSTRAT, TOL,&
     APP_T, AQQ_T, APQ_T, C_T, T_T, JPAIRS, LROT_T, LROT0, NROT, INFO)
#endif
  IMPLICIT NONE
...
#else
  DOUBLE PRECISION, INTENT(OUT), TARGET :: APP_T(VPAIRS), AQQ_T(VPAIRS), APQ_T(VPAIRS), C_T(VPAIRS), T_T(VPAIRS)
  INTEGER, INTENT(OUT), TARGET :: LROT_T(VPAIRS)
#endif
  INTEGER, INTENT(OUT) :: JPAIRS(2,NPAIRS), LROT0(VTORSV)

#ifndef USE_INTEL
  DOUBLE PRECISION, CONTIGUOUS, POINTER :: APP(:), AQQ(:), APQ(:), C(:), T(:)
  INTEGER, CONTIGUOUS, POINTER :: LROT(:)
#endif
  INTEGER :: JS(JSMLEN), STEPS, NT_P, NTPP, BLASNT, NT_V, CYC, STP, PAIR, P, Q, I, J, LROT1, LROT2
  DOUBLE PRECISION :: FASTR(5)

  INTEGER, INTRINSIC :: IAND, MOD, SUM
#ifndef USE_IBM
