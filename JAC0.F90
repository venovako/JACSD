SUBROUTINE MYDJAC0(NPAIRS, VPAIRS, VTORS, VTORSV, M, N, G, LDG, V, LDV, MAXCYC, MAXTHR, JSTRAT, TOL,&
     APP, AQQ, APQ, C, T, JPAIRS, LROT, LROT0, NROT, INFO)
  ...
  DOUBLE PRECISION, INTENT(OUT) :: APP(VPAIRS), AQQ(VPAIRS), APQ(VPAIRS), C(VPAIRS), T(VPAIRS)
  INTEGER, INTENT(OUT) :: JPAIRS(2,NPAIRS), LROT(VPAIRS), LROT0(VTORSV)
  ...
  !DIR$ ASSUME_ALIGNED APP:MALIGN_B, AQQ:MALIGN_B, APQ:MALIGN_B, C:MALIGN_B, T:MALIGN_B
  ...
  CALL JSTRAT_INIT(JS, JSTRAT, N, INFO(2))
  IF (INFO(2) .NE. 0) RETURN
  ...
  DO CYC = 1, MAXCYC
     ...
     DO STP = 1, STEPS
        ...
        DO I = 1, VTORS
           ...
           !DIR$ VECTOR ALWAYS,ALIGNED
#else
           !TODO: $OMP SIMD SIMDLEN(D_VEC_LEN), ALIGNED(APP,AQQ,APQ,C,T,LROT:MALIGN_B)
!IBM* ASSERT(NODEPS)
!IBM* INDEPENDENT
#endif
           DO J = 1, D_VEC_LEN
              ...
              IF (ABS(APQ(PAIR+J)) .LT. ((C(PAIR+J) * T(PAIR+J)) * TOL)) THEN
                 LROT(PAIR+J) = 0
              ELSE
                 LROT(PAIR+J) = 1
              END IF
        ...
        DO I = 1, VTORS
           ...
           IF (LROT0(I) .GT. 0) THEN
              ...
              !DIR$ VECTOR ALWAYS,ALIGNED
#else
              !TODO: $OMP SIMD SIMDLEN(D_VEC_LEN), ALIGNED(APP,AQQ,APQ,C,T:MALIGN_B)
!IBM* ASSERT(NODEPS)
!IBM* INDEPENDENT
#endif
              DO J = 1, D_VEC_LEN
                 ...
                 ! Cotangent(2\phi)
                 T(PAIR+J) = C(PAIR+J) / (APQ(PAIR+J) * D_TWO)
                 ! Tangent(\phi)
                 !DIR$ FMA
...
SUBROUTINE DJAC0(FAST, M, N, G, LDG, V, LDV, MAXCYC, MAXTHR, JSTRAT, TOL, SIGMA,&
  ...
  DOUBLE PRECISION, INTRINSIC :: DBLE, EPSILON, SCALE
#ifndef USE_IBM
  DOUBLE PRECISION, INTRINSIC :: SIGN, SQRT
#endif
  ...
  CALL MYDJAC0(NPAIRS, VPAIRS, VTORS, VTORSV, M, N, G, LDG, V, LDV, MAXCYC, MYMTHR, JSTRAT, MYTOL,&
       WORK(1), WORK(1 + LW1), WORK(1 + 2 * LW1), WORK(1 + 3 * LW1), WORK(1 + 4 * LW1),&
