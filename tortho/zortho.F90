PROGRAM ZORTHO
  ...
  COMPLEX(KIND=c_double), ALLOCATABLE, TARGET :: U(:,:)
  ...
  DO J = 1, N
     DO I = 1, M
        xU(I,J) = CMPLX(REAL(U(I,J)), AIMAG(U(I,J)), WP)
     END DO
  END DO
  !$OMP END PARALLEL DO
  ...
  WRITE (OUTPUT_UNIT,1) '|| U^H U - I ||_F =', CNF
  FLUSH(OUTPUT_UNIT)
...
CONTAINS
  ...
  SUBROUTINE READCL(FN, M, N, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 3) ERROR STOP 'zortho.exe FN M N'
  ...
  SUBROUTINE BREAD_YU(FD, YU, M, N, SZ, INFO)
    ...
    COMPLEX(KIND=c_double), INTENT(OUT), TARGET :: YU(M,N)
