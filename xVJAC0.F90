...
!IBM* ALIGN(MALIGN_B, G,V, SIGMA, WORK,IWORK, WORK1,IWORK1)
  ...
  IF (INFO(2) .NE. 0) STOP 'xVJAC0.exe M N J T F'
  IF (M .LE. 0) STOP 'M <= 0'
  IF (N .LE. 0) STOP 'N <= 0'
  IF (N .GT. M) STOP 'N > M'
  ...
  LWORK = 0
  LIWORK = 0
  CALL VJAC0(FAST, M, N, G, LDG, V, LDV, MAXCYC, T, J, TOL, SIGMA, WORK1, LWORK, IWORK1, LIWORK, NROT, INFO)
  IF (INFO(1) .LT. 0) STOP 'VJAC0(workspace query)'
  IF (INFO(1) .GT. 0) THEN
     LWORK = INFO(1)
  ...
  CALL VJAC0(FAST, M, N, G, LDG, V, LDV, MAXCYC, T, J, TOL, SIGMA, WORK, LWORK, IWORK, LIWORK, NROT, INFO)
  CALL TIMER_STOP(CLK)
