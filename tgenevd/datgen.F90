...
CONTAINS
  ...
  SUBROUTINE SGENDAT(N, ISEED, QLAMBDA, QA, SA, QWORK, INFO)
    IMPLICIT NONE

    INTEGER, INTENT(IN) :: N, ISEED(4)
    REAL(KIND=WP), INTENT(IN) :: QLAMBDA(N)
    REAL, INTENT(OUT) :: SA(N,N)
    ...
    IF (INFO .NE. 0) RETURN
    IF (INFO .NE. 0) RETURN
    !$OMP PARALLEL DO DEFAULT(NONE) SHARED(N,QA,SA) PRIVATE(P,Q)
    DO Q = 1, N
       DO P = 1, N
          SA(P,Q) = REAL(QA(P,Q))
       END DO
  ...
  SUBROUTINE CGENDAT(N, ISEED, QLAMBDA, XA, CA, XWORK, INFO)
    IMPLICIT NONE

    INTEGER, INTENT(IN) :: N, ISEED(4)
    REAL(KIND=WP), INTENT(IN) :: QLAMBDA(N)
    COMPLEX, INTENT(OUT) :: CA(N,N)
    ...
    IF (INFO .NE. 0) RETURN
    !$OMP PARALLEL DO DEFAULT(NONE) SHARED(N,XA,CA) PRIVATE(P,Q)
    DO Q = 1, N
       DO P = 1, Q-1
          CA(P,Q) = CMPLX(REAL(REAL(XA(P,Q))), REAL(AIMAG(XA(P,Q))))
       END DO
       CA(Q,Q) = CMPLX(REAL(REAL(XA(Q,Q))), 0.0)
       DO P = Q+1, N
          CA(P,Q) = CMPLX(REAL(REAL(XA(P,Q))), REAL(AIMAG(XA(P,Q))))
       END DO
    END DO
