...
2   WRITE (ERROR_UNIT,'(2A)') 'BIO_CLOSE   :', TRIM(MSG)
  ...
  SUBROUTINE BIO_WRITE_I1(U, N, A, INFO)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: U, N, A(N)
    INTEGER, INTENT(OUT) :: INFO
    ...
    RETURN
3   WRITE (ERROR_UNIT,'(2A)') 'BIO_WRITE_I1:', TRIM(MSG)
    FLUSH(ERROR_UNIT)
...
4   WRITE (ERROR_UNIT,'(2A)') 'BIO_WRITE_D1:', TRIM(MSG)
  ...
  SUBROUTINE BIO_WRITE_D2(U, M, N, A, INFO)
    ...
    WRITE (UNIT=U, IOSTAT=INFO, IOMSG=MSG, ERR=5) A
    RETURN
5   WRITE (ERROR_UNIT,'(2A)') 'BIO_WRITE_D2:', TRIM(MSG)
    FLUSH(ERROR_UNIT)
  ...
  SUBROUTINE BIO_WRITE_Z2(U, M, N, A, INFO)
    ...
    RETURN
6   WRITE (ERROR_UNIT,'(2A)') 'BIO_WRITE_Z2:', TRIM(MSG)
    FLUSH(ERROR_UNIT)
  END SUBROUTINE BIO_WRITE_Z2

  SUBROUTINE BIO_READ_I1(U, N, A, INFO)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: U, N
    ...
    RETURN
7   WRITE (ERROR_UNIT,'(2A)') 'BIO_READ_I1 :', TRIM(MSG)
    FLUSH(ERROR_UNIT)
  ...
  SUBROUTINE BIO_READ_D1(U, N, A, INFO)
    ...
    READ (UNIT=U, IOSTAT=INFO, IOMSG=MSG, ERR=8) A
    RETURN
8   WRITE (ERROR_UNIT,'(2A)') 'BIO_READ_D1 :', TRIM(MSG)
    FLUSH(ERROR_UNIT)
