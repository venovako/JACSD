...
CONTAINS
  ...
  SUBROUTINE ZGENDAT(N, ISEED, QLAMBDA, XA, ZA, XWORK, INFO)
    ...
    !$OMP PARALLEL DO DEFAULT(NONE) SHARED(N,XA) PRIVATE(P,Q)
    DO Q = 1, N
       DO P = 1, N
          XA(P,Q) = XZERO
       END DO
    ...
    !$OMP PARALLEL DO DEFAULT(NONE) SHARED(N,XA,QLAMBDA) PRIVATE(Q)
    DO Q = 1, N, 2
       XA(Q+1,Q) = CMPLX(QLAMBDA(Q), QZERO, WP)
       XA(Q,Q+1) = CMPLX(QLAMBDA(Q+1), QZERO, WP)
    END DO
    !$OMP END PARALLEL DO
