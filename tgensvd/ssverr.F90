PROGRAM SSVERR
  ...
  IF (INFO .NE. 0) THEN
     ...
     ERROR STOP ' ERRSTATS'
  END IF
  WRITE (OUTPUT_UNIT,'(5(ES15.9E2,A),ES15.9E2)') MINAE,',',MAXAE,',',AVGAE,',',MINRE,',',MAXRE,',',AVGRE
  FLUSH(OUTPUT_UNIT)
...
CONTAINS
  ...
  SUBROUTINE QCSORT(N, A)
    ...
    INTEGER, INTENT(IN) :: N
    REAL(KIND=WP), INTENT(INOUT), TARGET :: A(N)

    EXTERNAL :: PVN_QSORT
    INTEGER(KIND=c_int), EXTERNAL :: QCCMP

    CALL PVN_QSORT(A, INT(N,c_size_t), INT(WP,c_size_t), C_FUNLOC(QCCMP))
  ...
  SUBROUTINE READCL(N, FTRUE, FCOMP, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 3) ERROR STOP 'ssverr.exe N FTRUE FCOMP'
