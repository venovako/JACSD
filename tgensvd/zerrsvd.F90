PROGRAM ZERRSVD
  ...
  FLUSH(OUTPUT_UNIT)
  RNF = PXGEMM(M, N, N, xU, M, xV, N, xG, M) / GNF
...
CONTAINS
  ...
  SUBROUTINE READCL(M, N, FNG, FNU, FNS, FNV, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 6) ERROR STOP 'zerrsvd.exe M N FNG FNU FNS FNV'
