PROGRAM DTGSJA_TEST
  ...
  DOUBLE PRECISION, ALLOCATABLE :: U(:,:),V(:,:),Q(:,:)
  !DIR$ ATTRIBUTES ALIGN:ALIGNB :: U,V,Q
  DOUBLE PRECISION, ALLOCATABLE :: ALPHA(:),BETA(:)
...
CONTAINS
  ...
  SUBROUTINE BREAD_YW(FD, YW, M, N, SZ, INFO)
    ...
    SZ = M * N * C_SIZEOF(D_ZERO)
    IF (BREAD(FD, C_LOC(YW), SZ, 0) .EQ. SZ) THEN
       INFO = 0
    ELSE ! error
       INFO = 1
    END IF
  ...
  SUBROUTINE BWRITE_SS(FD, SS, N, INFO)
    ...
    INFO = INT(BWRITE(FD, C_LOC(SS), INT(C_SIZEOF(SS)), 0))
